...
export const DataCard: React.FC<DataCardProps> = ({ label, value, variation, color }) => {
  ...
  let chipColor = variation.percentage === 0 ? 'Neutral' : variation.percentage > 0 ? 'Positive' : 'Negative'
  ...
  let labelValue = ''
  if (variation.value) {
    labelValue = `${positiveVariationValue}${Math.abs(variationValueParse)} (${positivePercentageValue}${Math.abs(variation.percentage)}%)`
  } else {
    labelValue = `(${Math.abs(variation.percentage)}%)`
  }

  return (
    <Paper elevation={8} sx={{ p: 3, borderRadius: 2, borderLeft: `8px solid ${MACROCATEGORIES_COLORS_MAP.get(color)}` }}>
      ...
      <Stack sx={{ mt: 2 }} direction={{ sm: 'row' }} spacing={3} alignItems="center">
        ...
        <Box sx={{ mt: { xs: 3, sm: 0 } }}>
          <Chip
            sx={{ borderRadius: 1, bgcolor: colorMapChip.get(variation.value ? chipColor :'Neutral') }}
            label={labelValue}
            size="small"
          />
